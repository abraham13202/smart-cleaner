"""
AI-powered data cleaning advisor using Google Gemini (FREE!).
...
class GeminiAdvisor:
    """
    AI-powered advisor using Google Gemini API (free tier).
    ...
    def __init__(self, api_key: str, model: str = "models/gemini-2.5-flash"):
        ...
        Args:
            api_key: Google API key from aistudio.google.com
            model: Gemini model to use (models/gemini-2.5-flash is free and fast)
        """
        if genai is None:
            raise ImportError(
...
{{
  "strategy": "mean|median|mode|cohort_mean|knn|drop|forward_fill|backward_fill",
...
Provide only valid JSON, nothing else."""
    ...
    def _parse_recommendation(
        ...
        try:
            ...
            # Gemini might add explanation before/after JSON
